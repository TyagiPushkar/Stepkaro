...
export default function ReportPage() {
    ...
    const fetchOrders = useCallback(async () => {
        try {
            ...
            const response = await fetch(`${API_BASE}/vender/get_vendor_orders.php`, {
                headers,
            });
        ...
        <div className="space-y-6">
            ...
            <div className="mt-4">
                {activeTab === "commission" ? (
                    <CommissionReport orders={orders} loading={loading} error={error} role="seller" />
                ) : (
                    <SalesReport orders={orders} loading={loading} error={error} role="seller" />
                )}
            </div>
        </div>
    );
