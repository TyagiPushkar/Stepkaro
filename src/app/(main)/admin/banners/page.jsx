...
export default function BannersPage() {
    ...
    const fetchBrands = async () => {
      try {
        ...
        const res = await fetch(
          "https://namami-infotech.com/Stepkaro/src/brands/get_all_brands.php",
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const toggleStatus = async (banner) => {
    try {
      ...
      if (result.success) {
        fetchBanners();
        showToast(
          `Banner ${Number(banner.status) === 1 ? "deactivated" : "activated"} successfully`,
        );
      } else {
        alert(result.message);
      }
  ...
  const handleExportFilteredCSV = () => {
    ...
    showToast(
      `Exported ${filteredBanners.length} filtered banners successfully`,
    );
  };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 hover:border-purple-300 transition-all">
          <p className="text-2xl font-bold text-emerald-600">
            {banners.filter((b) => Number(b.status) === 1).length}
          </p>
          <p className="text-xs text-gray-500">Active Banners</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4 hover:border-purple-300 transition-all">
          <p className="text-2xl font-bold text-red-500">
            {banners.filter((b) => Number(b.status) === 0).length}
          </p>
      ...
      <div className="flex justify-between items-center">
        <p className="text-sm text-gray-500">
          ...
          <span className="text-gray-900">
            {filteredBanners.length > 0 ? startIndex + 1 : 0}
          </span>{" "}
          to{" "}
          <span className="text-gray-900">
            {Math.min(endIndex, filteredBanners.length)}
          </span>{" "}
          of <span className="text-gray-900">{filteredBanners.length}</span>{" "}
          banners
        </p>
