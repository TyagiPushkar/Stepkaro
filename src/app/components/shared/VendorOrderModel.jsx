...
export const OrderDetailsModal = ({
  ...
  const paymentReceiptUrl = getImageUrl(
    order?.payment_ss || order?.receipt_image,
  );
          ...
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="inline-flex items-center rounded-lg bg-indigo-100 px-3 py-1.5 text-base font-extrabold font-mono text-indigo-700 shadow-sm">
                  #{order?.id}
                </span>
              </div>
              ...
              {items.map((item, idx) => {
                ...
                const commission =
                  item?.commission_type === "percentage"
                    ? (selling_price * Number(item?.commission || 0)) / 100
                    : Number(item?.commission || 0);

                // Settlement Price (Payout per Pair)
                const stlem_price = selling_price - commission;
                const qty = Number(item?.quantity || 0);
                const pairs = Number(item?.pairs_per_ctn || 0);
                // const stlem_price = Number(item?.selling_price || 0);
                const price = Number(item?.price || 0);
                const itemTotal =
                  item?.total_price ?? selling_price * pairs * qty;
                // Settlement Total (Payout Total)
                const stleTotal = stlem_price * pairs * qty;
                        ...
                        <div className="flex items-center gap-2 mt-0.5 text-[11px] text-slate-500 flex-wrap">
                          ...
                          <span>
                            ...
                            <span className="font-medium">
                              {formatCurrency(stlem_price)}
                            </span>
                          </span>
                          <span className="text-slate-300">|</span>
                          <span className="text-emerald-600 font-bold">
                            Stle. Total: {formatCurrency(stleTotal)}
                          </span>
                        </div>
                      </div>
